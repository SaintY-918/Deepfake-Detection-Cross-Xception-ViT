# train_cross_xception_vit_dual.py 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="訓練 Cross-Xception-ViT 的雙分支融合版")
    ...
    model = CrossXceptionViTDual(config, pretrained=True)
    print("--- 已載入雙分支模型 (CrossXceptionViTDual) ---")
    
    
    optimizer = torch.optim.AdamW(model.parameters(), lr=config['training']['lr'], weight_decay=config['training']['weight-decay'])

    # --- 根據設定檔動態建立 Scheduler ---
    scheduler_name = config['training'].get('scheduler', 'cosine') # 預設使用 cosine

    if scheduler_name.lower() == 'cosine':
        # 從 config 讀取 T_max，如果沒有就使用 opt.num_epochs
        t_max = config['training'].get('t_max', opt.num_epochs) 
        # 從 config 讀取 eta_min，如果沒有就使用 1e-6
    ...
    for t in range(opt.num_epochs):
        ...
        if avg_val_loss < previous_loss:
            ...
            model_save_path = os.path.join(MODELS_PATH, f"xception_vit_dual_best_{opt.dataset}.pth")
